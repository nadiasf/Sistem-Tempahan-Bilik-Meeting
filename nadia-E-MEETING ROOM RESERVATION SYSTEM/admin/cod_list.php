...
<!------------------Viewing Product List From DataBase-------------------->
<?php
//This block grabs whole list for viewing
$cod_list='';
$sql=mysql_query("select * from cod order by date_order desc");
$codcount=mysql_num_rows($sql);//count the out put amount
if($codcount>0){
	while($rows=mysql_fetch_array($sql)){
		...
		$date_order=strftime('%d/%b/%Y',strtotime($rows['date_order']));
		$cod_list.="<p style='border-bottom:1px dotted #F37232; '>$date_order | $id | $email&nbsp | $firstname | $lastname | $contactno | $address1 | $address2 | $state | $city | $zipcode | $order_name&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='cod_edit.php?pid=$id'>Edit</a>&nbsp;|&nbsp;<a href='cod_delete.php?deleteid=$id'>Delete</a><br/></p>";
	}
}else{
	$cod_list='You have no products listed in your store yet .';
}
?>
...
<title>COD (Cash On Delivery) List</title>
...
<a style="text-decoration:none; color:#00F;" href="cod_form.php">+ Add New COD Order</a>
</div>
<div class="admin" style="margin-left:20px;">
<h2>COD (Cash On Delivery) List</h2>
<p style="color:#666; border-bottom:1px solid #F37232;">Date Order | ID | Email | First Name  | Last Name | Contact No | Address 1 | Address 2 | State | City | Zip Code | Order Name</p>
<?php echo $cod_list;?>
</div>
