...
<h2 style="text-align:left;color:black;margin-left:70px ;font-family:Times New Roman">Summary</h2>
		...
		<form action="summary.php" method="post" style="margin-top:30px;">
				...
				<a href="payment.php?id=<?php if(isset($ref_number)){ echo $ref_number;} ?>" class="pay">Pay Now</a>
				 <a href="edit.php?id=<?php if(isset($ref_number)){ echo $ref_number;} ?>" class="pay">Edit</a> 
				<input type="submit" value="Cancel Order" name="delete" style="width:100px;margin-left:100px;margin-top:20px;height:30px;"  />				 
	...
	<?php
     ...
     if(isset($_POST['delete'])){
		$ref=$_POST["ref"];
		$del_room=mysql_query("delete  from  `room_reg` where order_no='".$ref."' ")or die(mysql_error());
		$del_utility=mysql_query("delete  from  `utilities` where order_no='".$ref."' ")or die(mysql_error());
		$del_refreshment=mysql_query("delete  from  `refreshment` where order_no='".$ref."' ")or die(mysql_error());
		if($del_room && $del_utility && $del_refreshment){
